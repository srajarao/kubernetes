...
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  ...
  if $SSH_CMD $SSH_USER@$NODE_IP "sudo /usr/local/bin/k3s-agent-uninstall.sh" > /dev/null 2>&1; then
        ...
        echo -e "✅"  # Print checkmark anyway, as uninstall may have partial success
      fi
    else
      echo -e "✅"  # Print checkmark if uninstall script doesn't exist (already uninstalled)
    fi
  fi
fi
step_increment
...
# -------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  # Determine binary name based on node type
  if [ "$NODE_TYPE" = "agx" ]; then
    BINARY="k3s-arm64"
  else
  ...
  K3S_REINSTALL_CMD="export K3S_TOKEN=\"$TOKEN\";
    scp -o StrictHostKeyChecking=no -o LogLevel=ERROR -i ~/.ssh/id_ed25519 sanjay@$TOWER_IP:/tmp/$BINARY /tmp/$BINARY;
...
step_05(){
# =========================================================================
# STEP 05: Systemd Service Override (force correct server/node IP) $NODE_DISPLAY
# =========================================================================
...
step_06(){
# -------------------------------------------------------------------------
# STEP 06: Create Registry Config Directory $NODE_DISPLAY
# -------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
...
step_07(){
# -------------------------------------------------------------------------
# STEP 07: Configure Registry for $NODE_DISPLAY
# -------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
...
step_08(){
# -------------------------------------------------------------------------
# STEP 08: Restart Agent After Registry Config $NODE_DISPLAY
# -------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
...
step_09(){
# --------------------------------------------------------------------------------
# STEP 09: Configure NVIDIA Runtime on $NODE_DISPLAY
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
...
step_11(){
# --------------------------------------------------------------------------------
# STEP 11: Clean up FastAPI $NODE_DISPLAY Docker Image Tags
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
...
step_12(){
# --------------------------------------------------------------------------------
# STEP 12: Build $NODE_DISPLAY Docker Image
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  # Determine dockerfile and directory based on node type
  if [ "$NODE_TYPE" = "agx" ]; then
...
step_13(){
# --------------------------------------------------------------------------------
# STEP 13: Tag $NODE_DISPLAY Docker Image
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
    ...
    sleep 5
    if sudo docker tag fastapi-$NODE_NAME:latest $REGISTRY_IP:$REGISTRY_PORT/fastapi-$NODE_NAME:latest > /dev/null 2>&1; then
      echo -e "[32m✅[0m"
    else
      echo -e "[31m❌[0m"
      exit 1
...
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
    ...
    sleep 5
    sudo docker push $REGISTRY_IP:$REGISTRY_PORT/fastapi-$NODE_NAME:latest
  else
    step_echo_start "s" "tower" "$TOWER_IP" "Pushing Docker image to registry..."
    sleep 5
    if sudo docker push $REGISTRY_IP:$REGISTRY_PORT/fastapi-$NODE_NAME:latest > /dev/null 2>&1; then
      echo -e "[32m✅[0m"
    else
      echo -e "[31m❌[0m"
      exit 1
...
step_15(){
# ------------------------------------------------------------------------
# STEP 15: Deploy FastAPI on $NODE_DISPLAY (CPU-only)
# ------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
  ...
  if sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml get svc fastapi-$NODE_NAME-nodeport -n default --ignore-not-found=true >/dev/null 2>&1; then
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: fastapi
        image: $REGISTRY_IP:$REGISTRY_PORT/fastapi-$NODE_NAME:latest
        ports:
        - containerPort: 8000
          name: http
        - containerPort: 8888
...
step_16(){
# --------------------------------------------------------------------------------
# STEP 16: $NODE_DISPLAY GPU CAPACITY VERIFICATION
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ]; then
...
step_17(){
# --------------------------------------------------------------------------------
# STEP 17: $NODE_DISPLAY GPU RESOURCE CLEANUP
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ] && [ "$GPU_AVAILABLE" = true ]; then
...
step_18(){
# --------------------------------------------------------------------------------
# STEP 18: $NODE_DISPLAY GPU-ENABLED AI WORKLOAD DEPLOYMENT
# --------------------------------------------------------------------------------
if [ "$INSTALL_AGX_AGENT" = true ] || [ "$INSTALL_NANO_AGENT" = true ] && [ "$GPU_AVAILABLE" = true ]; then
...
step_20(){
# --------------------------------------------------------------------------------
# STEP 20: DISPLAY $NODE_DISPLAY SERVICE ENDPOINTS
# --------------------------------------------------------------------------------
