...
echo "## Authentication (Spark1 → Others)   ##"
echo "########################################"
...
echo "Setting up passwordless SSH from Spark1 to Tower, Nano, AGX, and Spark2..."
echo "Target devices: Tower ($TOWER_IP), Nano ($NANO_IP), AGX ($AGX_IP), Spark2 ($SPARK2_IP)"
echo ""
...
if [ ! -f "$KEY_FILE" ]; then
    ...
    ssh-keygen -t $KEY_TYPE -f "$KEY_FILE" -N "" -C "${SSH_USER}@spark1-$(date +%Y%m%d)"
...
if [ -f "$SSH_CONFIG" ]; then
    ...
    awk '
    /^Host (tower-from-spark1|nano-from-spark1|agx-from-spark1|spark2-from-spark1)$/ { skip = 1; next }
    skip && /^Host / { skip = 0 }
    !skip { print }
    ' "$SSH_CONFIG" > "${SSH_CONFIG}.tmp"
    mv "${SSH_CONFIG}.tmp" "$SSH_CONFIG"
    echo "   Cleaned existing tower-from-spark1/nano-from-spark1/agx-from-spark1/spark2-from-spark1 entries from SSH config"
fi
...
Host tower-from-spark1
    HostName $TOWER_IP
    User $SSH_USER
    IdentitiesOnly yes
    IdentityFile $KEY_FILE

Host nano-from-spark1
    HostName $NANO_IP
    User $SSH_USER
    IdentitiesOnly yes
    IdentityFile $KEY_FILE

Host agx-from-spark1
    HostName $AGX_IP
    User $SSH_USER
    IdentitiesOnly yes
    IdentityFile $KEY_FILE

Host spark2-from-spark1
    HostName $SPARK2_IP
    User $SSH_USER
    IdentitiesOnly yes
    IdentityFile $KEY_FILE
...
echo "      ssh tower-from-spark1     # Connect to Tower"
echo "      ssh nano-from-spark1      # Connect to Nano"
echo "      ssh agx-from-spark1       # Connect to AGX"
echo "      ssh spark2-from-spark1    # Connect to Spark2"

# Step 5: Final verification
echo ""
echo "5. Final verification of passwordless SSH..."

echo -n "   Testing 'ssh tower-from-spark1'... "
if ssh -o ConnectTimeout=5 -o BatchMode=yes tower-from-spark1 "hostname" 2>/dev/null; then
    echo "✅ SUCCESS"
else
    echo "❌ FAILED"
fi

echo -n "   Testing 'ssh nano-from-spark1'... "
if ssh -o ConnectTimeout=5 -o BatchMode=yes nano-from-spark1 "hostname" 2>/dev/null; then
    echo "✅ SUCCESS"
else
    echo "❌ FAILED"
fi

echo -n "   Testing 'ssh agx-from-spark1'... "
if ssh -o ConnectTimeout=5 -o BatchMode=yes agx-from-spark1 "hostname" 2>/dev/null; then
    echo "✅ SUCCESS"
else
    echo "❌ FAILED"
fi

echo -n "   Testing 'ssh spark2-from-spark1'... "
if ssh -o ConnectTimeout=5 -o BatchMode=yes spark2-from-spark1 "hostname" 2>/dev/null; then
    echo "✅ SUCCESS"
else
    echo "❌ FAILED"
fi
...
echo "   • ssh tower-from-spark1"
echo "   • ssh nano-from-spark1"
echo "   • ssh agx-from-spark1"
echo "   • ssh spark2-from-spark1"
