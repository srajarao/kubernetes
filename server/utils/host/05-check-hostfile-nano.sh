...
# Script: 05-check-hostfile-nano.sh
# Purpose: Check and update host file on nano node remotely
# This script can be run from any machine but executes on nano via SSH

# Configuration
NANO_HOST="nano"
SSH_USER="sanjay"

echo "=== Host File Check for Nano (Remote Execution) ==="
echo "Connecting to nano and checking /etc/hosts..."
echo

# Remote script to execute on nano
...
# Execute the script on nano via SSH
echo "Executing host file check on nano..."
ssh -o ConnectTimeout=10 "$SSH_USER@$NANO_HOST" "bash -s" << EOF
$REMOTE_SCRIPT
EOF
...
if [ $? -eq 0 ]; then
    echo
    echo "✅ Host file check completed successfully on nano"
else
    echo
    echo "❌ Host file check failed on nano"
    exit 1
fi