...
# Script: 06-check-hostfile-krithi.sh
# Purpose: Check and update host file on krithi node remotely
# This script can be run from any machine but executes on krithi via SSH

# Configuration
KRITHI_HOST="krithi"
SSH_USER="sanjay"

echo "=== Host File Check for Krithi (Remote Execution) ==="
echo "Connecting to krithi and checking /etc/hosts..."
echo

# Remote script to execute on krithi
...
# Execute the script on krithi via SSH
echo "Executing host file check on krithi..."
ssh -o ConnectTimeout=10 "$SSH_USER@$KRITHI_HOST" "bash -s" << EOF
$REMOTE_SCRIPT
EOF
...
if [ $? -eq 0 ]; then
    echo
    echo "✅ Host file check completed successfully on krithi"
else
    echo
    echo "❌ Host file check failed on krithi"
    exit 1
fi