...
# Script: 02-check-hostfile-spark1.sh
# Purpose: Check and update host file on spark1 node remotely
# This script can be run from any machine but executes on spark1 via SSH

# Configuration
SPARK1_HOST="spark1"
SSH_USER="sanjay"

echo "=== Host File Check for Spark1 (Remote Execution) ==="
echo "Connecting to spark1 and checking /etc/hosts..."
echo

# Remote script to execute on spark1
...
# Execute the script on spark1 via SSH
echo "Executing host file check on spark1..."
ssh -o ConnectTimeout=10 "$SSH_USER@$SPARK1_HOST" "bash -s" << EOF
$REMOTE_SCRIPT
EOF
...
if [ $? -eq 0 ]; then
    echo
    echo "✅ Host file check completed successfully on spark1"
else
    echo
    echo "❌ Host file check failed on spark1"
    exit 1
fi