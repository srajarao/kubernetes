...
# Script: 03-check-hostfile-spark2.sh
# Purpose: Check and update host file on spark2 node remotely
# This script can be run from any machine but executes on spark2 via SSH

# Configuration
SPARK2_HOST="spark2"
SSH_USER="sanjay"

echo "=== Host File Check for Spark2 (Remote Execution) ==="
echo "Connecting to spark2 and checking /etc/hosts..."
echo

# Remote script to execute on spark2
...
# Execute the script on spark2 via SSH
echo "Executing host file check on spark2..."
ssh -o ConnectTimeout=10 "$SSH_USER@$SPARK2_HOST" "bash -s" << EOF
$REMOTE_SCRIPT
EOF
...
if [ $? -eq 0 ]; then
    echo
    echo "✅ Host file check completed successfully on spark2"
else
    echo
    echo "❌ Host file check failed on spark2"
    exit 1
fi