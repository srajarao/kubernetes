...
# =============================================================================
# Remove K3s Agent from Spark1 Node
# =============================================================================
# This script removes the K3s agent service from the Spark1 node
...
SSH_USER="sanjay"
LOG_FILE="/home/sanjay/containers/kubernetes/rmvagent_spark1_$(date +%Y%m%d_%H%M%S).log"
...
# SSH command wrapper
ssh_cmd() {
    ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR -i /home/sanjay/.ssh/id_ed25519 "$SSH_USER@$SPARK1_IP" "$*"
}

# Header
echo "==================================================================================" | tee "$LOG_FILE"
echo "🗑️  REMOVE K3S AGENT FROM SPARK1" | tee -a "$LOG_FILE"
echo "==================================================================================" | tee -a "$LOG_FILE"
echo "Date: $(date)" | tee -a "$LOG_FILE"
echo "Target: Spark1 ($SPARK1_IP)" | tee -a "$LOG_FILE"
...
log "Stopping k3s-agent service on Spark1..."
...
log "Disabling k3s-agent service on Spark1..."
...
log "Running k3s-agent-uninstall.sh on Spark1..."
...
echo "==================================================================================" | tee -a "$LOG_FILE"
echo "Spark1 node has been removed from the Kubernetes cluster." | tee -a "$LOG_FILE"
echo "Log file: $LOG_FILE" | tee -a "$LOG_FILE"
echo "==================================================================================" | tee -a "$LOG_FILE"

echo -e "${GREEN}🎉 K3s agent removal from Spark1 completed successfully!${NC}"