...
# =============================================================================
# Remove K3s Agent from AGX Node
# =============================================================================
# This script removes the K3s agent service from the AGX node
...
SSH_USER="sanjay"
LOG_FILE="/home/sanjay/containers/kubernetes/rmvagent_agx_$(date +%Y%m%d_%H%M%S).log"
...
# SSH command wrapper
ssh_cmd() {
    ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR -i /home/sanjay/.ssh/id_ed25519 "$SSH_USER@$AGX_IP" "$*"
}

# Header
echo "==================================================================================" | tee "$LOG_FILE"
echo "🗑️  REMOVE K3S AGENT FROM AGX" | tee -a "$LOG_FILE"
echo "==================================================================================" | tee -a "$LOG_FILE"
echo "Date: $(date)" | tee -a "$LOG_FILE"
echo "Target: AGX ($AGX_IP)" | tee -a "$LOG_FILE"
...
log "Stopping k3s-agent service on AGX..."
...
log "Disabling k3s-agent service on AGX..."
...
log "Running k3s-agent-uninstall.sh on AGX..."
...
echo "==================================================================================" | tee -a "$LOG_FILE"
echo "AGX node has been removed from the Kubernetes cluster." | tee -a "$LOG_FILE"
echo "Log file: $LOG_FILE" | tee -a "$LOG_FILE"
echo "==================================================================================" | tee -a "$LOG_FILE"

echo -e "${GREEN}🎉 K3s agent removal from AGX completed successfully!${NC}"