...
# Network Setup Steps
echo -n "{s} [tower] [$TOWER_IP] 01/25. Setting up Tower network configuration..."
...
if ip addr show enp1s0f1 | grep -q "10.1.10.150"; then
  echo "  ✅ enp1s0f1 has IP 10.1.10.150"
  AGX_INTERFACE_AVAILABLE=true
else
  echo "  ❌ enp1s0f1 missing IP 10.1.10.150 (10G interface not connected)"
  echo "  ⚠️  Skipping AGX setup - 10G network unavailable"
fi
sleep 10

STEP=$((2 + 3))
# Start iperf3 server on Tower for network testing (only if 10G interface available)
echo -n "{s} [tower] [$TOWER_IP] 02/25. Starting iperf3 server for network testing... "
if [ "$AGX_INTERFACE_AVAILABLE" = true ]; then
  iperf3 -s -B 10.1.10.150 -D
  if [ $? -eq 0 ]; then
    echo -e "\033[32m✅\033[0m"
  else
...
if [ "$INSTALL_AGX_AGENT" = true ]; then
  echo -n "{a} [agx] [$AGX_IP] 03/25. Setting up AGX network configuration..."
  echo -e "\033[32m✅\033[0m"
else
  echo "{a} [agx] [$AGX_IP] 03/25. AGX network setup skipped (not enabled)"
fi


STEP=$((4 + 3))
if [ "$INSTALL_NANO_AGENT" = true ]; then
  echo -n "{a} [nano ] [$NANO_IP] 04/25. Setting up Nano network configuration..."
  echo -e "\033[32m✅\033[0m"
else
  echo "{a} [nano ] [$NANO_IP] 04/25. Nano network setup skipped (not enabled)"
fi
STEP=$((4 + 3))
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{s} [tower] [$TOWER_IP] 04/25. Uninstalling K3s server... "
  sleep 5
...
# Install Server
echo "{s} [tower] [$TOWER_IP] 05/25. Installing K3s server... "
echo -e "\033[32m$(printf '%.0s=' {1..80})\033[0m"
sleep 5
sudo curl -sfL https://get.k3s.io | sh -s - server
echo -e "\033[32m$(printf '%.0s=' {1..80})\033[0m"
echo -n "{s} [tower] [$TOWER_IP] 05/25. Installing K3s server... "
echo -e "\033[32m✅\033[0m"
STEP=$((6 + 3))
...
if [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
    ...
    echo -n "{a} [nano ] [$NANO_IP] 06/25. Uninstalling K3s agent on nano... "
    sleep 5
...
# Reinstall Nano Agent (via SSH)
if [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
    echo "Reinstalling Agent..."
    sleep 5
    ssh -o StrictHostKeyChecking=no sanjay@$NANO_IP "export K3S_TOKEN=\"$TOKEN\"; sudo curl -sfL https://get.k3s.io | K3S_URL=https://$TOWER_IP:6443 K3S_TOKEN=\$K3S_TOKEN sh -"
    wait_for_agent
  else
    echo -n "{a} [nano ] [$NANO_IP] 07/25. Reinstalling K3s agent on nano... "
    sleep 5
    if ssh -o StrictHostKeyChecking=no sanjay@$NANO_IP "export K3S_TOKEN=\"$TOKEN\"; sudo curl -sfL https://get.k3s.io | K3S_URL=https://$TOWER_IP:6443 K3S_TOKEN=\$K3S_TOKEN sh -" > /dev/null 2>&1; then
      wait_for_agent
      echo -e "\033[32m✅\033[0m"
    else
      echo -e "\033[31m❌\033[0m"
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [agx] [$AGX_IP] 05/25. Uninstalling K3s agent on agx... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [agx] [$AGX_IP] 06/25. Reinstalling K3s agent on agx... "
  sleep 5
...
"
    ...
    echo -n "{a} [agx] [$AGX_IP] 07/25. Configuring registry for agx... "
    sleep 5
...
# Configure Registry for Nano (via SSH)
if [ "$INSTALL_NANO_AGENT" = true ]; then
  ...
  if [ "$DEBUG" = "1" ]; then
    ...
    echo -n "{a} [nano ] [$NANO_IP] 07/25. Creating registry configuration directory... "
    sleep 5
  ...
  if [ "$DEBUG" = "1" ]; then
    ...
    echo -n "{a} [nano ] [$NANO_IP] 08/25. Adding insecure registry configuration... "
    sleep 5
...
EOF"
    ...
    echo -n "{a} [nano ] [$NANO_IP] 09/25. Fixing registry YAML syntax... "
    sleep 5
    ssh -o StrictHostKeyChecking=no sanjay@$NANO_IP "sudo tee /etc/rancher/k3s/registries.yaml > /dev/null <<EOF
configs:
  \"$REGISTRY_IP:$REGISTRY_PORT\":
...
"
    ...
    echo -n "{a} [nano ] [$NANO_IP] 10/25. Configuring containerd for registry... "
    sleep 5
...
" > /dev/null 2>&1
  ...
  if [ "$DEBUG" = "1" ]; then
    ...
    echo -n "{a} [nano ] [$NANO_IP] 11/25. Restarting K3s agent after registry config... "
    sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{s} [tower] [$TOWER_IP] 12/25. Restarting K3s server... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{s} [tower] [$TOWER_IP] 13/25. Applying taint to server node... "
  sleep 5
...
# Verify Node Status
if [ "$DEBUG" = "1" ]; then
  echo "{s} [tower] [$TOWER_IP] 14/25. Verifying Node Status..."
  sleep 5
  echo -e "\033[32m================================================================================\033[0m"
  sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml get nodes
  echo -e "\033[32m================================================================================\033[0m"
else
  echo -n "{s} [tower] [$TOWER_IP] 12/25. Verifying node status... "
  sleep 5
...
' | sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml apply -f -
  ...
  echo -n "{s} [tower] [$TOWER_IP] 15/25. Installing NVIDIA runtime class... "
  sleep 5
  echo 'apiVersion: node.k8s.io/v1
kind: RuntimeClass
metadata:
...
if [ "$DEBUG" = "1" ]; then
  ...
  if sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml apply -f nvidia-ds-updated.yaml > /dev/null 2>&1; then
    wait_for_gpu_capacity
    echo -n "{s} [tower] [$TOWER_IP] 16/25. Installing NVIDIA device plugin... "
    echo -e "\033[32m✅\033[0m"
  else
    echo -e "\033[31m❌\033[0m"
    exit 1
...
# Configure NVIDIA Runtime on Nano (via SSH)
if [ "$INSTALL_NANO_AGENT" = true ]; then
  if [ "$DEBUG" = "1" ]; then
    ...
    echo -n "{a} [nano ] [$NANO_IP] 16/25. Configuring NVIDIA runtime on agent... "
    sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [nano ] [$NANO_IP] 18/25. Copying files for Docker build... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [nano ] [$NANO_IP] 19/25. Building Docker image... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [nano ] [$NANO_IP] 20/25. Tagging Docker image... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{a} [nano ] [$NANO_IP] 21/25. Pushing Docker image to registry... "
  sleep 5
...
echo "{s} [tower] [$TOWER_IP] 22/25. Creating Deployment YAML..."
sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{s} [tower] [$TOWER_IP] 22/25. Deploying FastAPI application... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  echo -n "{s} [tower] [$TOWER_IP] 22/25. Monitoring pod status... "
  sleep 5
...
if [ "$DEBUG" = "1" ]; then
  ...
  STEP=$((24 + 3))
  echo -n "{s} [tower] [$TOWER_IP] 24/25. Reviewing and validating implementation... "
  sleep 5
  if sudo kubectl --kubeconfig /etc/rancher/k3s/k3s.yaml get pods -o wide > /dev/null 2>&1; then
    echo -e "\033[32m✅\033[0m"
  else
