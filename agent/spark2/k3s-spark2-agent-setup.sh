...
SPARK2_IP="${SPARK2_IP:-192.168.1.202}"                                        # SPARK2 device IP
...
EOF
        ...
        if [ -f "$TOKEN_CERT" ]; then
            ...
            if [ $INSTALL_STATUS -ne 0 ]; then
                ...
                if ! sudo iptables -C FORWARD -s $SPARK2_IP -d 192.168.5.0/24 -j ACCEPT 2>/dev/null; then
                    sudo iptables -I FORWARD -s $SPARK2_IP -d 192.168.5.0/24 -j ACCEPT
