...
# K3s Agent Setup Script for SPARK2
# Integrated from proven working setup_fastapi_spark1.sh with new architecture
# This script sets up SPARK2 as a k3s agent connected to the tower server
...
if [ -f "$SCRIPT_DIR/spark2-config.env" ]; then
    source "$SCRIPT_DIR/spark2-config.env"
    echo -e "${GREEN}Configuration loaded from spark2-config.env${NC}"
    echo -e "  Tower IP: ${TOWER_IP}"
    echo -e "  SPARK2 IP: ${SPARK2_IP}"
...
# Directories - will be overridden by spark2-config.env if present
TOKEN_DIR="${TOKEN_DIR:-/mnt/vmstore/spark2_home/containers/fastapi/.token}"  # SPARK2: Read Token from server
PROJECT_DIR="${PROJECT_DIR:-/home/sanjay/containers/fastapi}"              # SPARK2: Main project directory
IMAGE_DIR="${IMAGE_DIR:-/mnt/vmstore/spark2_home/containers/fastapi}"         # SPARK2: Save build images for server
TOWER_IP="${TOWER_IP:-192.168.10.1}"                                      # Tower server IP
SPARK2_IP="${SPARK2_IP:-10.1.10.202}"                                        # SPARK2 device IP
SPARK2_IFACE="${SPARK2_IFACE:-enP7s7}"                                       # SPARK2 management interface
...
function cleanup_k3s_agent_installation() {
    ...
    if [ -f "/usr/local/bin/k3s-agent-uninstall.sh" ]; then
        ...
        debug_msg "Deleting pods on spark2 node before k3s uninstall to free GPU resources"
        if [ -f "$KUBECONFIG_PATH" ] && kubectl --kubeconfig="$KUBECONFIG_PATH" get pods >/dev/null 2>&1; then
            echo -e "  Deleting pods on spark2 node to free GPU resources..."
            # Only delete pods scheduled on the spark2 node
            kubectl --kubeconfig="$KUBECONFIG_PATH" delete pods -l kubernetes.io/hostname=spark2 --force --grace-period=0 >/dev/null 2>&1
            print_result $? "  Deleted pods on spark2 node (GPU resources freed)"
        ...
        if [ -f "$TOKEN_CERT" ]; then
            ...
            if [ $INSTALL_STATUS -ne 0 ]; then
                ...
                if ! ip route show | grep -q "192.168.5.0/24 via $TOWER_IP"; then
                    sudo ip route add 192.168.5.0/24 via $TOWER_IP dev $SPARK2_IFACE metric 100
