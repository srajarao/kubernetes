#!/bin/bash
clear
echo -e "${GREEN}Starting SPARK1 FastAPI agent setup...${NC}\n"
# Setup colors
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[0;33m'
...
# Directories
TOKEN_DIR="/mnt/vmstore/spark1_home/containers/fastapi/.token"  # SPARK1: Read Token from server
PROJECT_DIR="/home/sanjay/containers/fastapi"                # SPARK1: Main project directory
IMAGE_DIR="/mnt/vmstore/spark1_home/containers/fastapi"         # SPARK1: Save build images for server
...
function cleanup_k3s_agent_installation() {
    ...
    if [ -f "$KUBECONFIG_PATH" ] && command -v kubectl >/dev/null 2>&1; then
        ...
        if [ "$DEBUG" -eq 1 ]; then
            echo -e "  Attempting forceful cleanup of old 'fastapi-spark1' resources... $TICK"
        fi
        # 1. Delete the Deployment and Service (ignore if not found)
        debug_msg "Deleting deployment fastapi-spark1"
        timeout 30s KUBECONFIG="$KUBECONFIG_PATH" kubectl delete deployment fastapi-spark1 --ignore-not-found >/dev/null 2>&1
...
EOF
        ...
        if sudo k3s ctr images list | grep -q "192.168.10.1:5000/fastapi_spark1"; then
...
function build_and_save_fastapi_image() {
    ...
    if [ -f "$PROJECT_DIR/dockerfile.online.req" ]; then
        ...
        if [ "$BUILT_IMAGE" = true ]; then
            ...
            docker tag fastapi-spark1:latest 192.168.10.1:5000/fastapi-spark1:latest
            print_result $? "  Tagged image for local registry"
            debug_msg "Pushing image to registry"
            docker push 192.168.10.1:5000/fastapi-spark1:latest
            PUSH_STATUS=$?
