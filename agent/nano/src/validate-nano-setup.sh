...
if kubectl --kubeconfig="$KUBECONFIG_PATH" get nodes | grep -q "$(hostname)"; then
    NODE_STATUS=$(kubectl --kubeconfig="$KUBECONFIG_PATH" get nodes --no-headers | grep "$(hostname)" | awk '{print $2}')
...
POD_NAME=$(kubectl --kubeconfig="$KUBECONFIG_PATH" get pods -l app=fastapi-nano -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
if [ -z "$POD_NAME" ]; then
    POD_NAME=$(kubectl --kubeconfig="$KUBECONFIG_PATH" get pods -l app=fastapi -o jsonpath='{.items[0].metadata.name}' 2>/dev/null)
fi

if [ -n "$POD_NAME" ]; then
    POD_STATUS=$(kubectl --kubeconfig="$KUBECONFIG_PATH" get pod "$POD_NAME" -o jsonpath='{.status.phase}' 2>/dev/null)
