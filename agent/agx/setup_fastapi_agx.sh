#!/bin/bash
clear
echo -e "${GREEN}Starting AGX FastAPI agent setup...${NC}\n"
# Setup colors
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[0;33m'
...
# Directories
TOKEN_DIR="/mnt/vmstore/agx_home/containers/fastapi/.token"  # AGX: Read Token from server
PROJECT_DIR="/home/sanjay/containers/fastapi"                # AGX: Main project directory
IMAGE_DIR="/mnt/vmstore/agx_home/containers/fastapi"         # AGX: Save build images for server
...
EOF
        ...
        if sudo k3s ctr images list | grep -q "192.168.10.1:5000/fastapi_agx"; then
...
function build_and_save_fastapi_image() {
    ...
    if [ -f "$PROJECT_DIR/dockerfile.online.req" ]; then
        ...
        if [ "$BUILT_IMAGE" = true ]; then
            ...
            docker tag fastapi-agx:latest 192.168.10.1:5000/fastapi-agx:latest
            print_result $? "  Tagged image for local registry"
            debug_msg "Pushing image to registry"
            docker push 192.168.10.1:5000/fastapi-agx:latest
            PUSH_STATUS=$?
