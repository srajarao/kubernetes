...
# Integrated from proven working setup_fastapi_agx.sh with new architecture
...
AGX_IP="${AGX_IP:-192.168.10.11}"                                        # AGX device IP
...
EOF
        ...
        if sudo k3s ctr images list | grep -q "${TOWER_IP}:5000/fastapi_agx"; then
